...
const Transactions: FC = () => {
  ...
  // Да, нужно передавать типы, но можно не передувать тип экшона каждый раз, так как он одинаковый во всех случаях

  const [{ items: transactions, loading }] = useItemsUploader<transactionType[], datesType>(
      "request",
      "transactions",
      "transactions",
      getTransactions,
        ...
        <Paper elevation={0} className={s.filters}>
          <LocalizationProvider dateAdapter={AdapterDateFns}>
            <DatePicker
                ...
                renderInput={(params) => <TextField {...params} className={s.datepicker} />}
            />
            <DatePicker
                openTo="day"
                views={["day", "month", "year"]}
  ...
  return <>
    <TableBody className={s.body}>
      {loading ? <TableSkeleton cols={8} />:
          transactions?.length ? transactions.map((item: transactionType, index: number) => (
              <>

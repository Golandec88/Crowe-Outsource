      ...
      <Grid item container>
        <Grid item xs={6}>
          <Table className={s.table}>
            ...
            <TableBody>
              {receivedFiles &&
                receivedFiles.files.items.map((el, index) => {
                  return (
                    <TableRow key={index} className={s.files}>
                      <TableCell
                        sx={{ padding: "5px" }}
                        style={{ padding: 20 }}
                      >
              ...
              <TableRow>
                <TablePagination
                  ...
                  onPageChange={(event, page) =>
                    handleChangePage(event, page, "user")
                  }
                  rowsPerPage={userRowsPerPage}
                  onRowsPerPageChange={(event) =>
                    handleChangeRowsPerPage(event, "user")
                  }
        ...
        <Grid item xs={6}>
          <Table className={s.table}>
            ...
            <TableBody>
              {staffUsers &&
                staffUsers.items.map((staff, index) => {
                  return (
                    <TableRow key={index}>
                      <TableCell style={{ padding: 0 }}>
                        ...
                        >
                          ...
                          {sentFiles && sentFiles.files.items?.length
                            ? sentFiles.files.items.map((files, index) => {
                                return (
                                  <AccordionDetails
                                    key={index}
                                    className={s.files}
                                  >
                                    ...
                                    <div>
                                      <IconButton
                                        color={"error"}
                                        onClick={(event) =>
                                          onStaffDelete(files.guid, event)
                                        }
                                      >
                                        <ClearIcon />
                                      </IconButton>
                          ...
                          <Table>
                            <TableBody>
                              <TableRow>
                                <TablePagination
                                  count={
                                    sentFiles ? sentFiles.files.totalCount : 0
                                  }
                                  page={staffPage}
                                  rowsPerPageOptions={[5, 10, 15, 20]}
                                  onPageChange={(event, page) =>
                                    handleChangePage(event, page, "staff")
                                  }
                                  rowsPerPage={staffRowsPerPage}
                                  onRowsPerPageChange={(event) =>
                                    handleChangeRowsPerPage(event, "staff")
                                  }
                                  backIconButtonProps={{ color: "secondary" }}
                                  nextIconButtonProps={{ color: "secondary" }}
                                  showFirstButton={true}
                ...
                <TableCell style={{ borderBottom: "none" }}>
                  <form className={s.form}>
                    ...
                    <input
                      ...
                      name="files"
                      multiple
                      onChange={(item) => staffUserUploadFilesHandler(item)}
                    ></input>
                  </form>
                </TableCell>
                <TablePagination
                  ...
                  rowsPerPageOptions={[5, 10, 15, 20]}
                  onPageChange={(event, page) =>
                    handleChangePage(event, page, "staffCount")
                  }
                  rowsPerPage={staffAmountRowsPerPage}
                  onRowsPerPageChange={(event) =>
                    handleChangeRowsPerPage(event, "staffCount")
                  }
                  backIconButtonProps={{ color: "secondary" }}
                  nextIconButtonProps={{ color: "secondary" }}
                  showFirstButton={true}
